...
export function desenharProdutoCarrinhoSimples(idProduto, idContainerHtml, quantidadeProduto){
    ...
    <p class="flex gap-10 text-green-700 text-lg">R$: ${produto.preco} <span class="text-green-900 text-lg">Total R$: ${produto.preco * quantidadeProduto}</span>
    </p>
    </div>
    <div class="flex text-slate-950 items-end absolute bottom-0 right-2 text-lg">
    <p id='quantidade-${produto.id}' class='ml-2 pb-3'>${quantidadeProduto}</p>
    </div>
    `
