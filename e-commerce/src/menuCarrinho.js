...
export function inicializarCarrinho(){
    ...
    const botaoPararCheckout = document.getElementById('finalizar-compra')

    botaoFecharCarrinho.addEventListener('click', fecharCarrinho)
    botaoAbrirCarrinho.addEventListener('click', abrirCarrinho)
    botaoPararCheckout.addEventListener('click', irParaCheckout)
}
...
function desenharProdutoNoCarrinho(idProduto){
  ...
  <button id='decrementar-produto-${produto.id}' class='mb-3'>-</button>
  <p id='quantidade-${produto.id}' class='ml-2 mb-3'>${idsProdutoCarrinhoComQuantidade[produto.id]}</p>
  <button id='incrementar-produto-${produto.id}' class='ml-2 mb-3'>+</button>
  </div>
  `
